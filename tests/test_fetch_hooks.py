...
def test_fetch_and_stage_adds_tags_null(monkeypatch):
    store = _due_store()
    monkeypatch.setattr("helpers.collect_hooks", lambda **kw: [dict(_SAMPLE_HOOK)])
    monkeypatch.setattr("helpers.refresh_due", lambda s, n: True)
...
def test_fetch_and_stage_noop_when_refresh_not_due(monkeypatch):
    store = _due_store()
    monkeypatch.setattr("helpers.refresh_due", lambda s, n: False)
    fetch_and_stage(store)
...
def test_fetch_and_stage_noop_on_all_duplicates(monkeypatch):
    store = _due_store()
    monkeypatch.setattr("helpers.collect_hooks", lambda **kw: [])
    monkeypatch.setattr("helpers.refresh_due", lambda s, n: True)
    fetch_and_stage(store)
    assert store["collections"] == []
...
def test_fetch_and_stage_network_failure_with_cache_is_graceful(monkeypatch):
    ...
    monkeypatch.setattr("helpers.collect_hooks", _fail)
    monkeypatch.setattr("helpers.refresh_due", lambda s, n: True)
...
def test_fetch_and_stage_does_not_modify_existing_collections(monkeypatch):
    ...
    monkeypatch.setattr("helpers.now_utc", lambda: datetime(2026, 1, 2, 12, 0, 0, tzinfo=timezone.utc))
    monkeypatch.setattr("helpers.collect_hooks", lambda **kw: [dict(_SAMPLE_HOOK)])
    monkeypatch.setattr("helpers.refresh_due", lambda s, n: True)
    fetch_and_stage(store)
...
def test_main_exits_zero_on_success(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("helpers.collect_hooks",
